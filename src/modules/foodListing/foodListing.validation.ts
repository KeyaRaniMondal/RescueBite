...
const foodCategoryValues = Object.values(FoodCategory) as [
	string,
	...string[],
];
const foodStatusValues = Object.values(FoodStatus) as [string, ...string[]];

const CreateFoodListingZodSchema = z.object({
	foodName: z
...
const UpdateFoodListingZodSchema = z
	.object({
		...
		quantity: z
			...
			.positive("Quantity must be greater than zero")
			.optional(),
		unit: z.string("Unit must be a string").min(1, "Unit is required").optional(),
		price: z
			.number("Price must be a number")
			.nonnegative("Price cannot be negative")
			.optional(),
